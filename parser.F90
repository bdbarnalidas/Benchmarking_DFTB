...
contains
  ...
  subroutine parseHSDInput(input,dynamics)
    integer :: dynamics
    ...
    call readHamiltonian(tmp, input%ctrl, input%geom, input%slako,dynamics)

    call getChildValue(root, "Driver", tmp, "", child=child, &
    ...
    case ("steepestdescent")
      ...
      if (ctrl%tLatOpt) then
        ...
        call convertByMul(char(modifier), pressureUnits, child, &
            & ctrl%pressure , .false., tChanged)
!        if (tChanged) then
!          call setChildValue(child, "", ctrl%pressure, .true.)
!        end if
      ...
      call getChildValue(node, "MovedAtoms", buffer2, "1:-1", child=child, &
          &multiple=.true.)
      call convAtomRangeToInt(char(buffer2), geom%specieNames, geom%species, &
          &child, ctrl%indMovedAtom)

      !call getChild(node, "MovedAtoms", child, requested=.false.)
      !if (.not. associated(child)) then ! no moved atoms set
      !  call setChild(node, "MovedAtoms", child)
      !  if (ctrl%tLatOpt) then
      !    ctrl%tCoordOpt = .false. ! we are moving the lattice but not internal
      !    ! coordinates
      !  else
      !    call setChildValue(child, "Range", (/ 1, -1 /))
      !    ctrl%tCoordOpt = .true. ! lattice and internal coordinates
      !  end if
      !else
      !  ctrl%tCoordOpt = .true.
      !end if
      !call getValueFromListOrRange(child, ctrl%indMovedAtom, mvalue=nAtom)
...
!        end if
    ...
    case("secondderivatives") ! currently only numerical derivatives of forces
      ...
      call getChildValue(node, "Delta", ctrl%derivDelta, 1.0E-4_dp, &
          & modifier=modifier, child=field)
      call convertByMul(char(modifier), lengthUnits, field, ctrl%derivDelta, &
          &.false., tChanged)
!      if (tChanged) then
!        call setChildValue(field, "", ctrl%derivDelta, .true.)
!      end if
    ...
    case ("velocityverlet")
      ...
      case ("berendsen")
        ...
        case ("temperatureprofile")
          call readTemperatureProfile(value2, child2, char(modifier), ctrl)
        case default
        ...
        if (associated(child2)) then
          ...
          if (associated(child2)) then
            ...
            call convertByMul(char(modifier), timeUnits, child3, &
                & ctrl%BarostatStrength , .false., tChanged)
      ...
      case ("none")
        ...
        if (ctrl%tReadMDVelocities) then
          ...
          ! velocities, we don't need a temperature, so just set it to something
          ! 'safe'
...
!          if (tChanged) then
!            call setChildValue(field, "", ctrl%tempAtom, .true.)
!          end if
        end if
      case default
      ...
      if (geom%tPeriodic) then
        ...
        if (.not. associated(child)) then
          ...
          call convertByMul(char(modifier), pressureUnits, child2, &
              & ctrl%pressure , .false., tChanged)
!          if (tChanged) then
!            call setChildValue(child2, "", ctrl%pressure, .true.)
!          end if
          ...
          if (associated(child2)) then
            ...
            if (associated(child2)) then
              ...
              call convertByMul(char(modifier), timeUnits, child3, &
                  & ctrl%BarostatStrength , .false., tChanged)
!              if (tChanged) then
!                call setChildValue(child2, "", ctrl%BarostatStrength, .true.)
!              end if
    ...
    if (char(buffer) == "") then
      ...
      if (len(modifier) > 0) then
        ...
        call convertByMul(char(modifier), VelocityUnits, child, &
            & tmpVelocities, .false., tChanged)
        if (tChanged) then
          call setChildValue(child, "", tmpVelocities, .true.)
        end if
      end if
      call destroy(realBuffer)
  ...
  subroutine readHamiltonian(node, ctrl, geo, slako,dynamics)
    integer :: dynamics
    type(fnode), pointer :: node
    type(control), intent(inout) :: ctrl
    ...
    select case (char(buffer))
    case ("dftb")
      call readDFTBHam(node, ctrl, geo, slako,dynamics)
    case default
  ...
  !!* @param geo   Geometry structure to be filled
  !!* @param slako Slater-Koster structure to be filled
  subroutine readDFTBHam(node, ctrl, geo, slako,dynamics)
    type(fnode), pointer :: node
    type(control), intent(inout) :: ctrl
    type(TGeometry), intent(in) :: geo
    type(slater), intent(inout) :: slako
    ...
    logical :: tLower, tChanged, tExist, tOrbResolved
    ...
    ALLOCATE_(angShells, (geo%nSpecie))
    do iSp1 = 1, geo%nSpecie
      call init(angShells(iSp1))
      call getChildValue(child, geo%specieNames(iSp1), value, child=child2)
      call getNodeName(value, buffer)
      select case(char(buffer))
      case("selectedshells")
        call init(lStr)
    ...
    INITALLOCATE_PARR(slako%orb%nShell, (geo%nSpecie))
    INITALLOCATE_PARR(slako%orb%nOrbSpecie, (geo%nSpecie))
    INITALLOCATE_PARR(slako%orb%nOrbAtom, (geo%nAtom))
    slako%orb%mOrb = 0
    slako%orb%mShell = 0
    do iSp1 = 1, geo%nSpecie
      slako%orb%nShell(iSp1) = 0
      slako%orb%nOrbSpecie(iSp1) = 0
      do ii = 1, len(angShells(iSp1))
        ...
        do jj = 1, nShell
          slako%orb%nOrbSpecie(iSp1) = slako%orb%nOrbSpecie(iSp1) &
              &+ 2 * angShell(jj) + 1
        end do
      end do
    end do
    slako%orb%mShell = maxval(slako%orb%nShell)
    slako%orb%mOrb = maxval(slako%orb%nOrbSpecie)
    slako%orb%nOrbAtom(:) = slako%orb%nOrbSpecie(geo%species(:))
    slako%orb%nOrb = sum(slako%orb%nOrbAtom)

    INITALLOCATE_PARR(slako%orb%angShell, (slako%orb%mShell, geo%nSpecie))
    INITALLOCATE_PARR(slako%orb%iShellOrb, (slako%orb%mOrb, geo%nSpecie))
    INITALLOCATE_PARR(slako%orb%posShell, (slako%orb%mShell+1, geo%nSpecie))
    ...
    !! Slater-Koster files
    ALLOCATE_(skFiles, (geo%nSpecie, geo%nSpecie))
    do iSp1 = 1, geo%nSpecie
      do iSp2 = 1, geo%nSpecie
    ...
    case ("type2filenames")
      ...
      do iSp1 = 1, geo%nSpecie
        ...
        do iSp2 = 1, geo%nSpecie
          if (tLower) then
            elem2 = tolower(geo%specieNames(iSp2))
          else
            elem2 = geo%specieNames(iSp2)
          end if
    ...
    ALLOCATE_(repPoly, (geo%nSpecie, geo%nSpecie))
    ...
    !! Use old repulsive summation (buggy for small periodic systems,
    !! where atom interacts with its periodic image).
    call getChildValue(node, "OldRepulsiveSum", ctrl%useBuggyRepSum, .false.)


    call getChildValue(node, "OrbitalResolvedSCC", tOrbResolved, .false.)
    ...
    call readSKFiles(skFiles, geo%nSpecie, slako, slako%orb, &
        &angShells, tOrbResolved, skInterMeth, repPoly)

    !WRITE(6,*) "skfiles",skfiles
    WRITE(6,*) "repoly",reppoly
    do iSp1 = 1, geo%nSpecie
    ...
    ifSCC: if (ctrl%tSCC) then
      ...
      call getChildValue(node, "MaxSCCIterations", ctrl%maxIter, 100)
      ...
      if (associated(child2)) then
        ...
        call convertByMul(char(modifier), EFieldUnits, child3, &
            & ctrl%EFieldStrength, .false., tChanged)
        ...
        call convertByMul(char(modifier), freqUnits, child3, &
            & ctrl%EFieldOmega, .false., tChanged)
      ...
      case ("colinear")
        ctrl%tSpin = .true.
        ctrl%t2Component = .false.
        call getChildValue(value, "UnpairedElectrons", ctrl%nrSpinPol)
        if (.not. ctrl%tReadChrg) then
      ...
      if (ctrl%tSpin) then
        INITALLOCATE_PARR(ctrl%W, \
        (slako%orb%mShell, slako%orb%mShell, geo%nSpecie))
        call getChild(node, "SpinConstants", child)
        do iSp1 = 1, geo%nSpecie
          call getChildValue(child, geo%specieNames(iSp1), &
              &ctrl%W(:slako%orb%nShell(iSp1), :slako%orb%nShell(iSp1), &
              &iSp1))
        end do
      end if
    ...
    if (.not. associated(child)) then
      ...
      call getChildValue(child, "Dual", ctrl%tDualSpinOrbit, .false.)

      INITALLOCATE_PARR(ctrl%xi,(slako%orb%mShell,geo%nSpecie))
      ctrl%xi = 0.0_dp
      do iSp1 = 1, geo%nSpecie
        call getChildValue(child, geo%specieNames(iSp1), &
        ...
        call convertByMul(char(modifier), energyUnits, child2, &
            & ctrl%xi(:slako%orb%nShell(iSp1),iSp1), .false., tChanged)
!        if (tChanged) then
!          call setChildValue(child2, "", ctrl%xi(:slako%orb%nShell(iSp1),iSp1),&
!              & .true.)
!        end if
      end do
    end if
    ...
    !! Solver
    call getChildValue(node, "Eigensolver", value, "DivideAndConquer")
    call getNodeName(value, buffer)
    select case(char(buffer))
    case ("standard")
      ctrl%iSolver = 1
...
!        if (tChanged) then
!          call setChildValue(field, "", ctrl%tempElec, .true.)
!        end if
      end if
...
!        end if
      ...
      if (ctrl%iDistribFn < 1) then
        ...
        call detailedError(child, errorStr)!"Unsuported filling mode '" &
!            & //char(buffer)// "'")
      end if
    case default
      call getNodeHSDName(value, buffer)
    ...
    if (geo%tPeriodic) then
      ...
      case (textNodeName)
        ...
        if (len(modifier) > 0) then
          ...
          call setChildValue(child, "", kpts, .true.)
        end if
        INITALLOCATE_PARR(ctrl%kPoint, (3, ctrl%nKPoint))
        INITALLOCATE_PARR(ctrl%kWeight, (ctrl%nKPoint))
    ...
    if (.not. associated(child)) then
      ...
      INITALLOCATE_PARR(ctrl%nUJ,(geo%nSpecie))
      ctrl%nUJ = 0

      ALLOCATE_(lrN,(geo%nSpecie)) ! to hold list of U-J values for each atom
      ALLOCATE_(liN,(geo%nSpecie)) ! to hold count of U-J values for each atom
      ALLOCATE_(li1N,(geo%nSpecie))! to hold list of shells for each U-J block
                                   !  of values
      do iSp1 = 1, geo%nSpecie
        call init(lrN(iSp1))
        call init(liN(iSp1))
        call init(li1N(iSp1))
        call getChildren(child, trim(geo%specieNames(iSp1)), children)
        ...
        do ii = 1, ctrl%nUJ(iSp1)
          ...
          call convertByMul(char(modifier), energyUnits, child3, rTmp, &
              &.false., tChanged)
!          if (tChanged) then
!            call setChildValue(child3, "", rTmp, .true.)
!          end if
          if (rTmp < 0.0_dp) then
      ...
      INITALLOCATE_PARR(ctrl%UJ,(maxval(ctrl%nUJ),geo%nSpecie))
      ctrl%UJ = 0.0_dp
      INITALLOCATE_PARR(ctrl%niUJ,(maxval(ctrl%nUJ),geo%nSpecie))
      ctrl%niUJ = 0
      do iSp1 = 1, geo%nSpecie
      ...
      INITALLOCATE_PARR(ctrl%iUJ, \
        (maxval(ctrl%niUJ),maxval(ctrl%nUJ),geo%nSpecie))
      ctrl%iUJ = 0
      do iSp1 = 1, geo%nSpecie
        do ii = 1, ctrl%nUJ(iSp1)
    ...
    if (associated(value)) then
      INITALLOCATE_P(ctrl%pDispInp)
      call readDispersion(child, geo, ctrl%pDispInp)
    end if
    ! Can't be checked as hamiltonian is processed *before* driver is read.
    ...
    if ((ctrl%t3rd .or. ctrl%t3rdFull) .and. tOrbResolved) then
      call error("Third order DFTB only compatible with orbital non resolved &
          &SCC")
    end if
    if (ctrl%t3rd .or. ctrl%t3rdFull) then
      call getChild(node, "HubbardDerivs", child, requested=.true.)
      INITALLOCATE_PARR(ctrl%hubDerivs, (geo%nSpecie))
      do iSp1 = 1, geo%nSpecie
        call getChildValue(child, geo%specieNames(iSp1), ctrl%hubDerivs(iSp1))
      end do
      if (ctrl%t3rd) then
        INITALLOCATE_PARR(ctrl%thirdOrderOn, (geo%nAtom, 2))
        ctrl%thirdOrderOn(:,1) = 0.0_dp
        ctrl%thirdOrderOn(:,2) = ctrl%hubDerivs(geo%species)
      end if
    end if
  ...
  contains
    ...
    subroutine getInitialSpins(node, geo, nSpin, initSpins)
      ...
      if (associated(child2)) then
        ...
        do ii = 1, getLength(children)
          call getItem1(children, ii, child2)
          call getChildValue(child2, "Atoms", buffer2, child=child3, &
              &multiple=.true.)
          call convAtomRangeToInt(char(buffer2), geo%specieNames, &
              &geo%species, child3, pTmpI1)
  ...
  !!* @param nrSpecie Nr. of species in the system
  !!* @param slako Data type for slako information
  !!* @param orb Information about the orbitals in the system
  !!* @param angShells For every specie a list of rank one arrays. Each array
  ...
  subroutine readSKFiles(skFiles, nSpecie, slako, orb, angShells, orbRes, &
      &skInterMeth, repPoly)
    ...
    ASSERT((size(skFiles, dim=1) > 0) .and. (size(skFiles, dim=1) == nSpecie))
    ASSERT(all(shape(repPoly) == shape(skFiles)))

    INITALLOCATE_PARR(slako%skSelf, (orb%mShell, nSpecie))
    INITALLOCATE_PARR(slako%skHubbU, (orb%mShell, nSpecie))
    INITALLOCATE_PARR(slako%skOcc, (orb%mShell, nSpecie))
    INITALLOCATE_PARR(slako%mass, (nSpecie))
    slako%skSelf(:,:) = 0.0_dp
    slako%skHubbU(:,:) = 0.0_dp
    slako%skOcc(:,:) = 0.0_dp

    INITALLOCATE_P(slako%skHamCont)
    call init(slako%skHamCont, nSpecie)
    INITALLOCATE_P(slako%skOverCont)
    call init(slako%skOverCont, nSpecie)
    INITALLOCATE_P(slako%repCont)
    call init(slako%repCont, nSpecie)
  ...
  !!* @param sp1 Specie number for A (for error messages)
  !!* @param sp2 Specie number for B (for error messages)
  ...
  subroutine checkSKCompRepSpline(repIn1, repIn2, sp1, sp2)
    ...
    if (size(repIn1%xStart) /= size(repIn2%xStart)) then
      write(errorStr, "(A,I2,A,I2,A)") "Incompatible nr. of repulsive &
          &intervals for specie pair ", sp1, "-", sp2, "."
      call error(errorStr)
    end if
    ...
    if (maxval(abs(repIn1%spCoeffs - repIn2%spCoeffs)) > tolRep &
        ...
        &> tolRep) then
      write(errorStr, "(A,I2,A,I2,A)") "Incompatible repulsive spline &
          &coefficients for specie pair ", sp1, "-", sp2, "."
      call error(errorStr)
    end if
    if (maxval(abs(repIn1%expCoeffs - repIn2%expCoeffs)) > tolRep) then
      write(errorStr, "(A,I2,A,I2,A)") "Incompatible repulsive spline &
          &exp. coefficients for specie pair ", sp1, "-", sp2, "."
      call error(errorStr)
    end if
    if (abs(repIn1%cutoff - repIn2%cutoff) > tolRep) then
      write(errorStr, "(A,I2,A,I2,A)") "Incompatible repulsive spline &
          &cutoffs for specie pair ", sp1, "-", sp2, "."
      call error(errorStr)
    end if
  ...
  !!* @param angShells1 Angular momenta to pick from the SK-files for specie A
  !!* @param angShells2 Angular momenta to pick from the SK-files for specie B
  ...
  subroutine readOptions(node, ctrl)
    ...
    if (ctrl%tSCC) then
      ...
      call getChildValue(node, "MullikenAnalysis", ctrl%tMulliken, .false.)
    end if
    if (ctrl%tMD .or. ctrl%tGeoOpt .or. ctrl%tDerivs) then
      ctrl%tForces = .true.
    else
      call getChildValue(node, "CalculateForces", ctrl%tForces, .false.)
      if (ctrl%tSCC) then
        call getChildValue(node, "ConvergentForcesOnly", ctrl%tConvrgForces, &
            & .true.)
      end if
    end if
    call getChildValue(node, "WriteEigenvectors", ctrl%tEigenvecs, .false.)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine readDispersion(node, geo, input)
    ...
    select case (char(buffer))
    case ("slaterkirkwood")
      INITALLOCATE_P(input%pSlaKirkInp)
      input%iDisp = iSlaterKirkWood
      call readDispSlaKirk(dispModel, geo, input%pSlaKirkInp)
    case ("lennardjones")
      INITALLOCATE_P(input%pVdWUFFInp)
      input%iDisp = iVdWUFF
      call readDispVdWUFF(dispModel, geo, input%pVdWUFFInp)
    case default
  ...
  subroutine readDispSlaKirk(node, geo, input)
    ...
    logical :: tChanged
    real(dp), pointer :: cellVec(:,:), rCellVec(:,:), coords(:,:)
    integer, pointer :: img2CentCell(:), iCellVec(:)
    integer :: nAllAtom
    type(TNeighborList) :: neighs

    ALLOCATE_(tmpR2, (3, geo%nAtom))
    INITALLOCATE_PARR(input%polar, (geo%nAtom))
    INITALLOCATE_PARR(input%rWaals, (geo%nAtom))
    INITALLOCATE_PARR(input%charges, (geo%nAtom))
    ...
    case ("hybriddependentpol")
      ...
      tChanged = .true.
      ALLOCATE_(rCutoffs, (geo%nSpecie))
      ALLOCATE_(tmp2R2, (13, geo%nSpecie))
      do iSp1 = 1, geo%nSpecie
        call getChildValue(value, geo%specieNames(iSp1), value2, &
    ...
    if (tChanged) then
      call setChildValue(child, "", tmpR2, .true.)
    end if
    input%polar(:) = tmpR2(1,:)
  ...
  subroutine readDispVdWUFF(node, geo, input)
    ...
    INITALLOCATE_PARR(input%distances, (geo%nSpecie))
    INITALLOCATE_PARR(input%energies, (geo%nSpecie))
    call getNodeName(value, buffer)
    select case(char(buffer))
    case("uffparameters")
      do iSp = 1, geo%nSpecie
        call getUFFValues(geo%specieNames(iSp), input%distances(iSp), &
            &input%energies(iSp), found)
        if (.not. found) then
          call detailedError(value, "UFF parameters for species '" // geo&
              &%specieNames(iSp) // "' not found.")
        end if
      end do
    case default
      call setUnprocessed(value)
      do iSp = 1, geo%nSpecie
        call getChild(child, geo%specieNames(iSp), child2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine readTemperature(node, ctrl)
    ...
    call removeModifier(node)
    if (tChanged) then
      call setChildValue(node, "", ctrl%tempValues(1), .true.)
    end if
  ...
  subroutine readTemperatureProfile(node, parent, modifier, ctrl)
    type(fnode), pointer :: node, parent
    ...
    tChanged = .false.
    call convertByMul(modifier, energyUnits, node, ctrl%tempValues, .false., &
        &tChanged)
    call removeModifier(parent)
    if (any(ctrl%tempValues < minTemp)) then
      ctrl%tempValues = max(ctrl%tempValues, minTemp)
      tChanged = .true.
    end if
    if (tChanged) then
      call setChildValue(node, "", tmpC1, &
          &reshape(ctrl%tempSteps, (/ 1, size(ctrl%tempSteps) /)), &
          &reshape(ctrl%tempValues, (/ 1, size(ctrl%tempValues) /)), &
          &.true.)
    end if
    DEALLOCATE_(tmpC1)
